...
def predict_test(model, batch_size, sql_data, table_data, output_path):
    ...
    for st in tqdm(range(len(sql_data)//batch_size+1)):
        ...
        for sql_pred in sql_preds:
            # fw.writelines(json.dumps(sql_pred,ensure_ascii=False, cls=MyEncoder).encode('utf-8')+'\n')
            fw.writelines(json.dumps(sql_pred,ensure_ascii=False, cls=MyEncoder)+'\n')
    fw.close()
